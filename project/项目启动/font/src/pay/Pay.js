...
export default class Pay extends Component {
    ...
    componentDidMount() {
        ...
        let url = `http://49.235.251.57:8000/api/trade/` + tradeid;
        fetch(url)
            .then((res) => res.json())
            .then((res) => {
                this.setState({
            ...
            <div>
                <div>
                    <div style={{ display: 'flex', textAlign: 'center', background: 'linear-gradient(to right,#F55E7E, #F47B87, #F58B7F)', lineHeight: 2 }}>
                        ...
                        <span style={{ marginLeft:'30%', fontSize: 25, color: 'white' }}>
                            确认付款
                        </span>
                        {/* <Link to='/help'>
                ...
                <div style={{ textAlign: 'center', marginTop: '30%' }} >
                    <input style={{ textAlign: 'center', width: '60%', height: '45px', backgroundColor: '#f5838e', fontSize: 23, borderRadius: 20 ,color:'#ffffff'}} type='button' onClick={this.successToast} value='提 交' />
